...
If DriveSpaceFree("C:\") <3000 then
	WriteLog("На филиале " & GetValue("BaseName",$aArray) &  " заканчивается место на диске " & "C:\" ,"logDumpIB.txt",1)
	SendMail("На филиале " & GetValue("BaseName",$aArray) &  " заканчивается место на диске " & "C:\" ,GetValue("mailto",$aArray))
EndIf
WriteLog("Начало выполнения ","logDumpIB.txt",1)
...
;~ 		exit
;~ 		WriteLog("До выгона","logDumpIB.txt",1)
		...
		if StringLower(GetValue("RestartService",$aArray)) = "true" then
			...
			WriteLog("Остановка службы - " & $sServiceName,"logDumpIB.txt",1)
			Sleep(60000)
			_StartService("", $sServiceName)
			WriteLog("Запуск службы - " & $sServiceName,"logDumpIB.txt",1)
			Sleep(60000)

			$sServiceName1c="1C:Enterprise 8.2 Server Agent"
			_StopService("", $sServiceName1c)
			WriteLog("Остановка службы - " & $sServiceName1c,"logDumpIB.txt",1)
			Sleep(60000)
			_StartService("", $sServiceName1c)
			WriteLog("Запуск службы - " & $sServiceName1c,"logDumpIB.txt",1)
			Sleep(60000)
		EndIf
		...
		if Run($StrokaZapuska)=0 Then
			...
			Exit
		EndIf
		$MaksTimeCheck=7200000 ;до 2-х часов проверяем наличие файла result.txt
		$TekTimeCheck=0;
...
Else
	If FileExists(GetValue("PathDump",$aArray) & "log.txt") then
		...
		if $Poz<>0 then
			...
			WriteLog("Время загрузки на FTP " & $UploadTime & " минут","logDumpIB.txt",1)
			WriteLog("Время загрузки на FTP " & $UploadTime & " минут",$NameDump & ".txt",1)
		EndIf
		if $fl_DTUpload Then
...
if FileExists(GetValue("PathDump",$aArray) & $NameDump &".dt") then
	DeleteOldDT($aArray)
EndIf
WriteLog("Конец выполнения ","logDumpIB.txt",1)
...
;~ 	WriteLog("Создание v82.COMConnector" ,"logDumpIB.txt",1)
...
;~ 	_ArrayDisplay($arrayDB2)
	...
	For $i = 0 to $SizeDT -1
		...
		EndIf
		if $FileDelete Then
			WriteLog("Удален старый бекап - " & $arrayDB2[$i][1],"logDumpIB.txt",1)
...
Func FtpUpload($NameFile,$aArray,$FolderFTP,$flDeleteFile)
	...
	if  $Conn=0 Then
		WriteLog("Не получилось подключится к FTP.","logDumpIB.txt",1)
	endIf
	$CurrentDir="backup/"
	...
	if _FTP_DirSetCurrent($Conn, $CurrentDir)=0 Then
		WriteLog("Не получилось установить текущий каталог FTP.","logDumpIB.txt",1)
	EndIf
...
;~ 		MsgBox(-1,"",@ERROR)
		if $FolderFTP = "\backup\DT\" then
			...
			WriteLog("Не удалось скопировать "& $NameFile &" на FTP.","logDumpIB.txt",1)
			Return False
		EndIf
	Else
		WriteLog("Скопирован " & $NameFile & " на FTP.","logDumpIB.txt",1)
	EndIf
	$Ftpc = _FTP_Close($Open)
