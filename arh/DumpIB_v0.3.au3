...
;~ проверка не запущен ли уже процесс, если запущен, новый не запускаем
$list = ProcessList("DumpIB.exe")
for $i = 1 to $list[0][0]	
	;msgbox(0, $list[$i][0], $list[$i][1])
	if $i >1 then 
	  WriteLog("Процесс уже запущен","logDumpIB.txt",1)
	  Exit
	EndIf
next
...
If not FileExists("config.ini") Then
	...
	FileWriteLine($file, "mailto=ящик на который отсылается письмо, в случае неудачи")
 	FileWriteLine($file, "TimeDump=00:20")
	FileClose($file)
EndIf    
...
;~ 	Exit	
	While 1
		if @HOUR=$Hours AND @MIN=$Minutes  then 
			$TimeStr=_Date_Time_GetLocalTime()
			$TimeStr=_Date_Time_SystemTimeToDateTimeStr($TimeStr)		
