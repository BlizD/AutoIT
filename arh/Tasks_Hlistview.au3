...
Global $hListView,$hListView_WorkCourse,$GUI,$GUI_Work,$Button_Handle,$Button_AddTask,$Button_EditTask, $Form_AddTask,$TaskNew_Edit,$Worker_Combo,$Add_Button,$Button_AnswerTask,$ImageList
...
Global $sUsername="taskadmin", $sPassword="bdksat",  $sServer="A07-gw"
...
;~ 	Exit
	...
	$Height_WorkCourse=310
	$Left_FormCourse=$Width_FormCourse
	$Width_Form_AddTask=523
	$Top_FormCourse=0
	$IconSize=1
	if @OSVersion="WIN_7" Then
		$Height_Form_AddTask=110
		$Width_Form_AddTask=$Width_Form_AddTask + 12
		$Top_FormAddTask=352
		$Left_FormAddTask=5
		...
		$Width_FormCourse=$Width_FormCourse+10
...
;~ 	TraySetState (4)
	...
	$GUI = GUICreate("Задачи " & @UserName & " - " & $CurrentUser & "-" & $sDatabase, 517, 320,$Left_Form,10)
...
;~ 	$hListView = _GUICtrlListView_Create($GUI, "", 2, 25, 517, 240)
 	$hListView = GUICtrlCreateListView( "", 2, 25, 517, 240)
	_GUICtrlListView_SetExtendedListViewStyle($hListView, BitOR($LVS_EX_FULLROWSELECT, $LVS_EX_SUBITEMIMAGES, $LVS_EX_GRIDLINES, $LVS_EX_DOUBLEBUFFER))
	_GUICtrlListView_RegisterSortCallBack($hListView)
	$ImageList = _GUIImageList_Create(5,12)
;~ 	_GUIImageList_Add($ImageList, _GUICtrlListView_CreateSolidBitMap($hListView, 0xFF0000, 16, 16))
;~ 	_GUIImageList_Add($ImageList, _GUICtrlListView_CreateSolidBitMap($hListView, 0xFFFFFF, 16, 16))
;~ 	_GUIImageList_AddBitmap($ImageList, "C:\Voskl.bmp")
;~ 	_GUIImageList_AddIcon($ImageList, 206)
	_GUIImageList_AddIcon($ImageList, "C:\Voskl.ico")
...
;~     _GUICtrlListView_SetBkColor($hListView, $COLOR_SILVER)
	...
	; Add columns
	_GUICtrlListView_InsertColumn($hListView, 0, "Дата", 67,3,-1,false)
	_GUICtrlListView_InsertColumn($hListView, 1, "Статус", 69,3,-1,false)
	_GUICtrlListView_InsertColumn($hListView, 2, "Задача", 255)
	_GUICtrlListView_InsertColumn($hListView, 3, "Кто", 110)
	...
	$hListView_WorkCourse = _GUICtrlListView_Create($GUI_Work, "", 2, $Top_hListView_WorkCourse,$Width_FormCourse-7, $Height_WorkCourse)
...
;~ 	_GUICtrlListView_InsertColumn($hListView_WorkCourse, 1, "№", 20)
	...
	$Form_AddTask = GUICreate("Добавить задачу", $Width_FormCourse, $Height_Form_AddTask,-1,-1,$WS_SIZEBOX)
...
;~ 	$TaskNew_Edit = _GUICtrlEdit_Create($Form_AddTask, "", 2, 2, 440,40,$ES_MULTILINE+	)
	$TaskNew_Edit = _GUICtrlEdit_Create($Form_AddTask, "", 2, 2, 440+68,40,BitOr($ES_MULTILINE,$ES_WANTRETURN,$ES_AUTOVSCROLL))
	_GUICtrlEdit_SetLimitText($TaskNew_Edit, 500)
	GUICtrlCreateLabel("Кто:", 2, 53)
 	$Worker_Combo=_GUICtrlComboBox_Create($Form_AddTask, "", 25,45, 130, 100)
	GUICtrlCreateLabel("Статус:", 135, 53)
	$Status_Combo=_GUICtrlComboBox_Create($Form_AddTask, "", 195,45, 100, 100)
	$Add_Button=_GUICtrlButton_Create($Form_AddTask,"", 300, 45,141+68,20)
...
;~ 	ConsoleWrite("AddTask $CurseOpen- "&$CurseOpen&" $AddTaskOpen- "& $AddTaskOpen & " $TypeProcess -" & $TypeProcess&@CRLF)
	...
	ElseIf $TypeProcess= "Edit" Then
		$Text_Task=""
		$Text_Task=_GUICtrlListView_GetItemText($hListView,$CurrentRow, 2)
		$Worker=_GUICtrlListView_GetItemText($hListView,$CurrentRow, 3)
		...
		$Status=_GUICtrlListView_GetItemText($hListView,$CurrentRow, 1)
...
;~ 							$Event_Text="Ход: " &  $TaskTextEdit & " для задачи: " &  _GUICtrlListView_GetItemText($hListView,$CurrentRow, 1)
							$Event_Text="Задача: " &  _GUICtrlListView_GetItemText($hListView,$CurrentRow, 2)
							$Koef=Ceiling(StringLen($Event_Text)/60)
;~ 							ConsoleWrite($Koef)
							While StringLen($Event_Text)<60*$Koef-1
								$Event_Text=$Event_Text &" "
							WEnd
							$Event_Text=$Event_Text & "Исполнитель: " &  _GUICtrlListView_GetItemText($hListView,$CurrentRow, 3)
...
;~ 							Exit
							$id_Ispolnitel=(_GUICtrlComboBox_FindString($Worker_Combo,_GUICtrlListView_GetItemText($hListView,$CurrentRow, 3))+1)
