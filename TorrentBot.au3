...
Func SynhroniseFolders($FolderISX,$FolderSYNC)
	...
	Dim $FolderISX[1][3],$FolderSYNC[1][3]
	GetFiles($PathDWND,$FolderISX,2,$CurrentIndexISX)
;~ 	ConsoleWrite($PathDropBox)
	GetFiles($PathDropBox,$FolderSYNC,2,$CurrentIndexSync)
;~ 	_ArrayDisplay($FolderSYNC)
;~ 	Exit
;~ 	ConsoleWrite($CurrentIndexISX)
	$SizeArray=$CurrentIndexISX ;$CurrentIndexSync
	Dim $ArrayFolders[$SizeArray][5]
	$id_AF=0
;~ 	_ArraySort($FolderISX,0,0,0,2)
;~ 	_ArraySort($FolderSYNC,0,0,0,2)
;~ 	_ArrayDisplay($FolderISX)
;~ 	_ArrayDisplay($FolderSYNC)
;~ 	_ArraySort(ByRef $avArray[, $iDescending = 0 [, $iStart = 0 [, $iEnd = 0 [, $iSubItem = 0]]]])
;~ 	Exit
	$Find_index=0
	For $id_C = 0 to  $CurrentIndexISX-1
		$ArrayFolders[$id_AF][0]=StringReplace($FolderISX[$id_C][0],$PathDWND,"")
		$ArrayFolders[$id_AF][1]=""
;~ 		For $id_DBX = 0 to $CurrentIndexSync-1
;~ 			if StringReplace($FolderISX[$id_C][0],$PathDWND,"")=StringReplace($FolderSYNC[$id_DBX][0],$PathDropBox,"")  Then
;~ 				$ArrayFolders[$id_AF][1]=StringReplace($FolderSYNC[$id_DBX][0],$PathDropBox,"")
;~ 				$ArrayFolders[$id_AF][2]=False
;~ 				$ArrayFolders[$id_AF][3]=False
;~ 				$ArrayFolders[$id_AF][4]=$FolderISX[$id_C][1]
;~ 				$FolderSYNC[$id_DBX][0]=""
;~ 			EndIf
;~ 		Next

;~  		$Find_index= _ArrayBinarySearch($FolderSYNC,$FolderISX[$id_C][2],0,0)
		$Find_index=_ArraySearch($FolderSYNC, $FolderISX[$id_C][2] , $Find_index ,0 ,0, 1 ,1 ,2)

;~ 		_ArrayDisplay($FolderSYNC)
;~ 		ConsoleWrite($Find_index & @CRLF)
		If $Find_index <> -1 then
			$ArrayFolders[$id_AF][1]=$FolderSYNC[$Find_index][2]
			$ArrayFolders[$id_AF][2]=False
			$ArrayFolders[$id_AF][3]=False
			$ArrayFolders[$id_AF][4]=$FolderISX[$Find_index][1]
			$FolderSYNC[$Find_index][0]=""
		EndIf
...
;~ 	Exit
	...
	For $id_DBX = 0 to $CurrentIndexSync-1
;~ 		ConsoleWrite("$FolderSYNC[$id_DBX][0]=" & $FolderSYNC[$id_DBX][0] & @CRLF)
		if $FolderSYNC[$id_DBX][0]<>"" And StringInStr($FolderSYNC[$id_DBX][0],"logTorrentBot.txt")=0 Then
			$SizeArray=$SizeArray+1
			ReDim $ArrayFolders[$SizeArray][5]
			$ArrayFolders[$id_AF][0]=""
			$ArrayFolders[$id_AF][1]=StringReplace($FolderSYNC[$id_DBX][0],$PathDropBox,"")
			$ArrayFolders[$id_AF][2]=False
			$ArrayFolders[$id_AF][3]=True
;~ 			ConsoleWrite("$id_AF=" & $id_AF & " $id_DBX=" &$id_DBX & @CRLF)
			$ArrayFolders[$id_AF][4]=$FolderSYNC[$id_DBX][1]
			$id_AF=$id_AF+1
		EndIf
	Next
	ConsoleWrite("" & _NowTime() & @CRLF)
...
;~ 	Exit
	For $id = 0 to $SizeArray-1
		if $ArrayFolders[$id][2] then
			if $ArrayFolders[$id][4] = "file" then
				$file = FileOpen($PathDropBox & $ArrayFolders[$id][0], 1)
;~ 				FileWriteLine($logfile,$Time & "-" & $Text)
	...
	For $id = 0 to $SizeArray-1
		if $ArrayFolders[$id][3] then
			if $ArrayFolders[$id][4] = "file" then
				FileDelete ($PathDropBox & $ArrayFolders[$id][1])
			Else
		...
		For $id = 1 to $FileList[0]
			...
			if StringInStr($FileAtr,"D") then
				if $Type = 2 then
					...
					$array[$CurrentIndex][1]="folder"
					$array[$CurrentIndex][2]=$FileList[$id]
...
;~ 						WriteLog("Скопирован торрент " & $arrayTorrents[$CurrentIndex][2],"logTorrentBot.txt",1)
				...
				Else
					if $Type = 2 then
						ReDim $array[$CurrentIndex+1][3]
						$array[$CurrentIndex][0]=$PathFolder & $FileList[$id]
						$array[$CurrentIndex][1]="file"
						$FileName=StringReplace($PathFolder &$FileList[$id],$PathDropBox,"")
						$FileName=StringReplace($FileName,$PathDWND,"")

						$array[$CurrentIndex][2]=$FileName
						$CurrentIndex=$CurrentIndex+1
					EndIf
				EndIf
			EndIf
